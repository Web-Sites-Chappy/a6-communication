...
export default function ArticleBody({ blocks }: { blocks: BlogBlock[] }) {
  // Découpe en sections à chaque H2 : le Reveal (animation d'entrée) se
  // déclenche section par section plutôt que sur un bloc unique de
  // 9 000-15 000px, dont le seuil de visibilité n'est jamais atteint sur mobile.
  const sections: BlogBlock[][] = [];
  blocks.forEach((block) => {
    if (block.kind === "h2" || sections.length === 0) {
      sections.push([block]);
    } else {
